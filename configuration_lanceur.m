...
fprintf("nb_iter\tVp\t\t m_i \t\t\t\t\t theta\t\t\t\t\t deltaV \t ||R_tf|| - R_c\t R_tf*V_tf\t\n");
while true 
fprintf("%d\t", nb_iter_total);
...
%fprintf("Nombre d'iteration pb d'etagement : %d", nb_iter);

m_e_opt = m_e_all(:, end);
f_opt = f_all(end);
fprintf("%f %f %f\t", m_e_opt);
...
fprintf("%f\t", deltaV);
fprintf("%f\t", norm(R_tf) - R_c);
fprintf("%f\t", R_tf'*V_tf);

THETA_init = theta_opt;

if (abs(deltaV) < 0.1 && abs(R_tf'*V_tf) < 0.01 && abs(norm(R_tf) - R_c) < 0.01)
    break;
end
fprintf("\n")
end
%fprintf("Nombre iter total est: %i\n", nb_iter_total);
%print_result(nb_iter_THETA, nb_eval_THETA_all, THETA_all, f_THETA_all, c_THETA_all, lambda_THETA_all,  Grad_L_norm_THETA_all, rho_THETA_all)
